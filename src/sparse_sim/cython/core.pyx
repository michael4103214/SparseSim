...
cdef extern from "khash.h":
    ...
    khiter_t kh_get_outer_product_hash(kh_outer_product_hash_t *h, unsigned int key)
    khiter_t kh_put_outer_product_hash(kh_outer_product_hash_t *h, unsigned int key, int *ret)
...
cdef class PauliSum:
    ...
    def __mul__(self, right):
        ...
        elif isinstance(right, DensityMatrix):
            return density_matrix_pauli_sum_left_multiplication(self, right)
        else:
...
cdef class OuterProduct:
    ...
    def __add__(self, right):
        if isinstance(right, OuterProduct):
            ...
            raise TypeError(f"OuterProduct + {type(right)} is not defined")

    def __rmul__(self, left):
        if isinstance(left, numbers.Number):
            return outer_product_scalar_multiplication(self, left)
        else:
            raise TypeError(f"{type(left)} * OuterProduct is not defined")
...
cdef class DensityMatrix:
    ...
    def __mul__(self, right):
        if isinstance(right, DensityMatrix):
            ...
            raise TypeError(f"DensityMatrix * {type(right)} is not defined")

    def __rmul__(self, left):
        if isinstance(left, numbers.Number):
            return density_matrix_scalar_multiplication(self, left)
        else:
            raise TypeError(f"{type(left)} * DensityMatrix is not defined")
...
def density_matrix_CPTP_evolution(PauliSum H, list[PauliSum] Ls, DensityMatrix dm, double complex t):
    ...
    c_Ls = <PauliSumC **> malloc(num_Ls * sizeof(PauliSumC *))
    if not c_Ls:
