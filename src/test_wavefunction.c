...
void test_inner_product(void) {
    ...
    printf("(%s) * (%s) = %lf + %lfi\n", bra_str, ket_str, creal(inner_prod), cimag(inner_prod));
...
void test_wavefunction_speed(void) {  
    ...
    printf("\nRunning Wavefunction Speed Test with %u qubits, %u Slater determinants, %u Pauli sum operations.\n",
    ...
    for (i = 0; i < num_sdets; i++) {
        ...
        coef = (rand() / (double) RAND_MAX) + (rand() / (double) RAND_MAX) * (double complex) I;
    ...
    printf("Final number of Slater determinants is %u showing a 2^%.2f increase.\n",
